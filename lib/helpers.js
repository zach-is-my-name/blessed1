...
helpers.stripTags = function (text) {
  if (!text) return "";
  return helpers.stripAnsi(text.replace(/{(\/?)([\w\-,;!#]*)}/g, ""));
};

helpers.stripAnsi = function (text) {
  return text.replace(helpers.ansiRegexpAll, "");
};

helpers.ansiRegexpAll =
  /[\u001B\u009B][[\]()#;?]*(?:(?:(?:(?:;[-a-zA-Z\d\/#&.:=?%@~_]+)*|[a-zA-Z\d]+(?:;[-a-zA-Z\d\/#&.:=?%@~_]*)*)?\u0007)|(?:(?:\d{1,4}(?:;\d{0,4})*)?[\dA-PR-TZcf-nq-uy=><~]))/g;
helpers.ansiRegexpOne =
