...
Screen.prototype.render = function () {
  ...
  // chjj: Possibly get rid of .dirty altogether.
  // chjj: Could possibly drop .dirty and just clear the `lines` buffer every
...
Screen.prototype.draw = function (start, end) {
  ...
  const hasLinkFlag = attr => {
    const hasLinkOpen =  Boolean((attr >> 18) & 32)
    const hasBell =  Boolean((attr >> 18) & 64)
    if (hasLinkOpen || hasBell) {
      return true
    } 
    return false
  }
  ...
  for (y = start; y <= end; y++) {
    ...
    for (x = 0; x < line.length; x++) {
      ...
      // holdTODO check if it does the tput stuff on the first code or just the closing code
      // Optimize by comparing the real output
      // buffer to the pending output buffer.
      // holdTODO check and find origin of lx value
      if (data === o[x][0] && ch === o[x][1]) {
        ...
        if (this.tput.strings.parm_right_cursor) {
          // tput line 664: test for param and change bracket direction

         // holdTODO Actually, could be the cursor move code.'\x1B[28C\x1B[1;' Investigate.  
      ...
      if (data !== attr) {
        if (attr !== this.dattr) {
          if (hasLinkFlag(attr) === false) {
            out += "\x1b[m";
          }
        }
      }
        if (data !== this.dattr) {
          // doneTODO add with link decimal 
          // Issue: check if contains link (and bell) by decoding the "binary" (actually stored as decimal number) as done in series of if statements below
          // part1 link open 
          bellCode: if ((data >> 18) & 32) {
            out +="\x1b]"
          } else if ((data >> 18) & 64) {  
            break bellCode
          } else {
          ...
          //`\u001b]8;;http://example.com/\u0007Example.com\u001b]8;;\u0007`
          // doneTODO still need to add the flag here in this case,  out += 8;;
          // doneTODO 0x07 where appropriate
...
debugger;
  // TODO modify to accomodate OSC style
  if (out[out.length - 1] === ";") out = out.slice(0, -1);
