...
Element.prototype.render = function () {
  ...
  for (y = yi; y < yl; y++) {
    ...
    for (x = xi; x < xl; x++) {
      ...
      ch = content[ci] || bch;  //ch: 'x'
      if ((attr >> 18) & 64 || (attr >> & 32) & 32) {
        attr = (def >> 18) & 0x1ff;
      }
      ci++
      // content: '\x1B]8;;http://example.com/\x07Example.com\x1B]8;;\x07'
      while (ch === "\x07") {
        if ( /\x07$/.test(content.substring(ci-1)) || /\x07.*\x1b]/.test(content.substring(ci-1))) {
          attr = this.screen.attrCode("\x07", attr, dattr);
          ch = content[ci] || bch; // ch: 'E'
          ci++; //advance outside ch 
        }
      }
      while (ch === "\x1b") {
        let linkOpenRe = /^\x1b\]8;;/
        if (linkOpenRe.test(content.substring(ci-1))) {
          ...
          ci = ci + c[0].length - 1;
          attr = this.screen.attrCode(c[0], attr, dattr);//ch: \u001b  attr: 17039359  content[ci]: "]"
