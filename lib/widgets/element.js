...
Element.prototype.parseContent = function (noTags) {
  ...
  if (
    ...
    if (!noTags) {
      content = this._parseTags(content);  // tags to OSC
    }

    this._clines = this._wrapContent(content, width);  //wrap with OSC
    this._clines.width = width;
    this._clines.content = this.content;
...
Element.prototype._wrapContent = function (content, width) {
  ...
  main: for (; no < lines.length; no++) {
    ...
    while (line.length > width) {
      ...
      /// 如果判定转义字符移除之后，内容没有溢出，那么就不需要额外处理了
    ...
    for (x = xi; x < xl; x++) {
      ...
      while (ch === "\x07") {
        if ( /\x07$/.test(content.substring(ci-1)) || /\x07.*\x1b]/.test(content.substring(ci-1))) {
          attr = this.screen.attrCode("xx\x07;", attr, dattr);
          if (
            this.parent._isList &&
      ...
      while (ch === "\x1b") {
        ...
        if (linkOpenRe.test(content.substring(ci-1))) {
          ...
          ch = content[ci] || bch;
          ci++; } else if ((c = helpers.ansiRegexpOne.exec(content.substring(ci - 1)))) {
            ci += c[0].length - 1;
            attr = this.screen.attrCode(c[0], attr, dattr); // \x1B[1m
            // Ignore foreground changes for selected items.
            if (
              this.parent._isList &&
              this.parent.interactive &&
