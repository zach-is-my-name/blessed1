...
function Element(options) {
  ...
  // Chjj: Possibly move this to Node for onScreenEvent('mouse', ...).
...
Element.prototype.setText = function (content, noClear) {
  content = content || "";
  content = helpers.stripAnsi(content);
  return this.setContent(content, noClear, true);
};

Element.prototype.getText = function () {
  return helpers.stripAnsi(this.getContent());
};
...
Element.prototype.parseContent = function (noTags) {
  ...
  if (
    ...
    const re = /\x1b\]8;;(?<url>[(http(s)?):\/\/(www\.)?a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)),\x07(?<text>(?!\x1b).*?)\x1b\]8;;\x07/g

    content = content
      .replace(re, '{osc8}$<url>{st}$<text>{osc8}{st}')
    ...
    if (!noTags) {
      content = this._parseTags(content);  // tags to ansi
    }

    this._clines = this._wrapContent(content, width);  //wrap with ansi
    this._clines.width = width;
    this._clines.content = this.content;
...
Element.prototype._parseTags = function (text) { //convert tag to ansi
...
Element.prototype._parseAttr = function (lines) {
  ...
  for (j = 0; j < lines.length; j++) {
    ...
    for (i = 0; i < line.length; i++) {
      // doneTODO adjust to fit OSC 
      if (line[i] === "\x07") {
        if ( /\x07$/.test(line.substring(i)) || /\x07.*\x1b]/.test(line.substring(i))) {
        attr = this.screen.attrCode("\x07", attr, dattr);
        i += c[0].length - 1;
        }
      }

      if (line[i] === "\x1b") {
        let linkOpenRe = /^\x1b\]8;;/
        if (linkOpenRe.test(line.substring(i))) {
          c = linkOpenRe.exec(line.substring(i))
          attr = this.screen.attrCode(c[0], attr, dattr);
          i += c[0].length - 1;
        } else if ((c = helpers.ansiRegexpOne.exec(line.substring(i)))) {
          attr = this.screen.attrCode(c[0], attr, dattr);
          i += c[0].length - 1;
        }
      }
...
Element.prototype._align = function (line, width, align) {
  ...
  var cline = helpers.stripAnsi(line),
    len = cline.length,
    s = width - len;
...
Element.prototype._wrapContent = function (content, width) {
  ...
  main: for (; no < lines.length; no++) {
    ...
    while (line.length > width) {
      /**match all escape characters */
      const ansiEscList = line.match(helpers.ansiRegexpAll);
      // how does reduce work?
      // what are its params? 
      const ansiEscCharCount = ansiEscList ? ansiEscList.reduce((t, s) => t + s.length, 0) : 0;
      // If it is determined that the content does not overflow after the escape character is removed, then no additional processing is required.
      if (line.length - ansiEscCharCount <= width) {
        break;
      }
...
Element.prototype._getCoords = function (get, noscroll) {
  ...
  if (el && !noscroll) {
    ...
    // Chjj: Figure out how to fix base (and cbase to only
...
Element.prototype.render = function () {
  ...
  var lines = this.screen.lines, //empty usually
    xi = coords.xi,
    xl = coords.xl,
    yi = coords.yi,
    yl = coords.yl,
  ...
  for (y = yi; y < yl; y++) {
    ...
    for (x = xi; x < xl; x++) {
      ...
      ch = content[ci] || bch;                      //content = _pcontent 
      if ((attr >> 18) & 64 || (attr >> 18) & 32) {
        attr = dattr
      }
      ci++

      while (ch === "\x07") {
        if ( /\x07$/.test(content.substring(ci-1)) || /\x07.*\x1b]/.test(content.substring(ci-1))) {
          attr = this.screen.attrCode("\x07", attr, dattr);
          if (
            this.parent._isList &&
      ...
      while (ch === "\x1b") {
        let linkOpenRe = /^\x1b\]8;;/
        if (linkOpenRe.test(content.substring(ci-1))) {
          c = linkOpenRe.exec(content.substring(ci - 1))
          ci = ci + c[0].length - 1;
          attr = this.screen.attrCode(c[0], attr, dattr);
          if (
            this.parent._isList &&
          ...
          ch = content[ci] || bch; //ch: \u0007
          ci++; 

          while (ch === "\x07") {
            if ( /\x07$/.test(content.substring(ci-1)) || /\x07.*\x1b]/.test(content.substring(ci-1))) {
              ci = ci + c[0].length - 1;
              attr = this.screen.attrCode("x\x07", attr, dattr);
              if (
                this.parent._isList &&
                this.parent.interactive &&
                this.parent.items[this.parent.selected] === this &&
...
// '\x1B]8;;http://example.com/\x07Example.com\x1B]8;;\x07'
