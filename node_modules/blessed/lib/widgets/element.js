    ...
    for (x = xi; x < xl; x++) {
      ...
      while (ch === "\x1b") {
        ...
        if (linkRe.test(content.substring(ci-1))) {
          ...
          let fullLinkRe = /\x1b\]8;;(?<url>[(http(s)?):\/\/(www\.)?a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*))\x07(?<text>(?!\x1b).*?)\x1b\]8;;\x07/g 

          c = helpers.fullLinkRe.exec(content.substring(ci - 1)) 
